...
sr_names = names(sr)
core_cols = c("CLIP","NUMBER OF BUILDINGS","NUMBER OF UNITS",
              "MAILING CITY","MAILING STATE","FIPS CODE","YEAR BUILT",
...
n_total = 152118538 # got from end of counts file
chunk_size=10**6
...
col_names_vec_alt = names(fread(file=hist_property_basics1_fname,nrows=0))
...
for (i in 1:n_chunks){
  ...
  data_i = fread(file = hist_property_basics1_fname,
                 #   nrows=100,
                 #  skip = 5,
                 # sep = "|",
...
hist_property_basics1_files = list.files("")
...
#write.csv(hpb,file=str_glue("{export_path}/data_dictionary.csv"))
