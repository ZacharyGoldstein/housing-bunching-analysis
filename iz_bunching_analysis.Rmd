...
df_iz = df_iz_raw %>% 
  select(`Program ID`,`Program Name`,`Government Name`,`Government Type`,
        ...
        `Year Program Adopted`,`Year Program Updated`,
         `Program Applicable Areas`,`Minimum Project Size`,
        `Minimum Set-aside`,`Highest Income Served`,
            `Lowest Income Served`,`(Rental) Single Income Requirement Detail`,
            `(Rental) Multiple Tiers of Income Targeting Detail`,
         `Incentive`) %>% 
  ...
  mutate(minimum_project_size = ifelse(minimum_project_size %in% c("Don't know","N/A","1 unit","2 units","3 units"),"missing",str_to_lower(minimum_project_size)),
    ...
    lowest_income_served_clean = 
           ifelse(str_detect(lowest_income_served,"% AMI"),
                  str_sub(lowest_income_served,1,3) %>% 
                    str_replace("%","") %>% 
                    as.numeric,lowest_income_served),
    minimum_set_aside_clean =
           str_extract(`minimum_set-aside`,pattern="[0-9.]+") %>% 
           as.numeric
    ) %>% 
...
df_analysis = df_merged %>% 
  ...
  select(situs_city,situs_county,situs_state,government_name,government_type,
        ...
        cl,lowest_income_served_clean,minimum_set_aside_clean) ####
df_analysis %>% 
  slice_sample(n=10)
df_analysis %>% dim
```
...
```{r}
# what percent is CL vs. supplemental databases?
df_units %>% group_by(cl) %>% summarize(round(n()/nrow(df_units),4))
```
...
```{r}
ggplot(df_units %>% filter(number_of_units<=50,number_of_units>=2),
...
df_units %<>% 
  mutate(threshold_group = case_when(threshold_num<=9~"A 5-9",
                                     threshold_num<=19~"B 10-19",
                                     threshold_num<=29~"C 20-29",
                                     TRUE~"D 30+"))
ggplot(df_units %>% filter(rel_units<=50,rel_units>=-50),
       aes(x=rel_units,fill=as.factor(threshold_group))) +
  geom_histogram(binwidth=1,color='white') + 
  labs(x = 'Units Relative to Threshold',
       y = 'Frequency',
       title = 'Relative Unit Count Frequencies by Threshold',
       fill = "Threshold")
```
...
Placebo checks
```{r}
for (x in c(-7,-5,-3,-1,1,3,5,7)){
  print(x)
  p = DCdensity(runvar=df_units %>% pull(rel_units),cutpoint=x,ext.out=F,
                  plot=F)
  print(p)
  print("----------------")
  }
...
df_units %>% 
  ...
  labs(y = 'Frequency', x = 'Relative Units',
       title = 'Relative Unit Count Distributions by Threshold')
```

#### Inference

```{r}
thresh_group_mcrary_results = data.frame(matrix(nrow=1,ncol=8))
names(thresh_group_mcrary_results) = c("group","n_buildings","n_programs","binsize","theta","se","p","z")
for (group in df_units$threshold_group %>% unique %>% sort){
  df_group = df_units %>% filter(threshold_group==group)
  n_buildings = df_group %>% nrow
  n_programs = df_group %>% select(government_name,situs_state) %>% n_distinct
  # Adjust binsize/bw to make it work and avoid errors
  if (group %in% c("A 5-9")){
      results = DCdensity(runvar=df_group %>% pull(rel_units),cutpoint=0,ext.out=T,
                      bin=.9,plot=T)
  }
  else{
    results = DCdensity(runvar=df_group %>% pull(rel_units),
                        cutpoint=0,ext.out=T,plot=T)
  }
  p = results$p %>% round(5)
  z = results$z %>% round(3)
  se = results$se %>% round(3)
  binsize = results$binsize %>% round(3)
  theta = results$theta %>% round(3)
  results_vec = c(group,n_buildings,n_programs,binsize,theta,se,p,z)
  thresh_group_mcrary_results = rbind(thresh_group_mcrary_results,results_vec) 
    
}
thresh_group_mcrary_results = thresh_group_mcrary_results %>% drop_na(group) %>% 
  mutate(across(-group,as.numeric))
thresh_group_mcrary_results %>% 
  kable %>% 
...
```{r}
thresh_group_mcrary_placebo_results = data.frame(matrix(nrow=1,ncol=4))
names(thresh_group_mcrary_placebo_results) = c("group","cutpoint","binsize","theta")
for (group in df_units$threshold_group %>% unique %>% sort){
  print("-------------------------")
  print("GROUP")
  print(group)
  df_group = df_units %>% filter(threshold_group==group)
  min_x = df_group %>% arrange(rel_units) %>% tail(nrow(df_group)-20) %>% pull(rel_units) %>% min
  max_x = df_group %>% arrange(rel_units) %>% head(nrow(df_group)-20) %>% pull(rel_units) %>% max
  max_x = min(max_x,100)
  cutpoints = seq.int(from=min_x+1,to=max_x-1,by=1)
  for (cut_x in cutpoints){
    #print("---")
    #print("CUTPOINT")
    #print(cut_x)
    binsize = 2*sd(df_group$rel_units)*length(df_group$rel_units)^(-.5)
    while (binsize > 0){
    tryCatch({
      #print("Binsize")
      #print(binsize)
      results = DCdensity(runvar=df_group %>% pull(rel_units),cutpoint=cut_x,
                            ext.out=T,plot=F,bin = binsize)
      theta = results$theta %>% round(3)
      results_vec = c(group,cut_x,binsize %>% round(3),theta)
      thresh_group_mcrary_placebo_results = rbind(thresh_group_mcrary_placebo_results,results_vec)
      break}
      , 
    warning = function(w) {
    #print(w)
...
thresh_group_mcrary_placebo_results = thresh_group_mcrary_placebo_results %>% drop_na(group) %>% 
  mutate(across(-group,as.numeric))
thresh_group_mcrary_placebo_results
```

```{r}
placebo_check_p_values = data.frame(matrix(nrow=1,ncol=3))
i = 0
for (group_i in thresh_group_mcrary_placebo_results$group %>% unique %>% sort){
  i = i+1
  df_group = thresh_group_mcrary_placebo_results %>% filter(group==group_i) %>% 
    mutate(abs_theta=abs(theta))
  theta_obs = thresh_group_mcrary_results %>% filter(group==group_i) %>% 
    first %>% pull(theta)
  plot = df_group %>% 
    ggplot(aes(x=theta)) +
    geom_histogram(color='white',fill='black') +
    geom_vline(xintercept=theta_obs,linetype='dashed',color='blue',size=1.5) +
    labs(title = str_glue("Placebo Theta Distribution: {group_i}"),
         x = "Theta",y = "Frequency")
  p = nrow(df_group %>% filter(abs_theta>abs(theta_obs))) / nrow(df_group)
  p_one_sided = nrow(df_group %>% filter(theta<theta_obs)) / nrow(df_group)
  placebo_check_p_values %<>% rbind(c(group_i,p,p_one_sided))
  print(p)
  print(plot)
}
```
```{r}
df_placebo = as.data.frame(placebo_check_p_values)
names(df_placebo) = c("Threshold Range","p (two-tailed)","p (one-tailed)")
#df_placebo[,'Threshold Range'] = names(placebo_check_p_values)
df_placebo %>% 
  drop_na() %>% 
  mutate(across(contains("p "),as.numeric)) %>% 
...
```{r}
bunching_cities_mod = bunching_cities_table %>% 
  full_join(df_unit_bunch %>% 
               select(situs_city,situs_state,lowest_income_served_clean,
                      minimum_set_aside_clean) %>% 
               distinct,by=c("situs_city","situs_state")) %>% 
  mutate(lowest_income_served_clean=as.numeric(lowest_income_served_clean))
bunching_cities_mod
```
```{r}
bunching_cities_mod %>% 
  filter(is.na(minimum_set_aside_clean)==F) %>% 
  ggplot(aes(x=minimum_set_aside_clean)) +
  geom_histogram(binwidth=2,color='white') +
  scale_x_continuous(limits=c(0,40)) +
  labs(x = "Minimum Set-Aside (% of total units)",
       y = "Number of Programs",
       title = "Distribution of Minimum Set-Aside Across Programs")
```

```{r}
bunching_cities_mod %>% 
  filter(is.na(lowest_income_served_clean)==F) %>% 
  ggplot(aes(x=lowest_income_served_clean)) +
  geom_histogram(binwidth=5,color='white') +
  labs(x = "Lowest Income Served (% Area Median Income)",
       title = 'Distribution of Lowest Income Served Across Programs',
       y = "Number of Programs")
```

```{r}
bunching_cities_mod %>% 
  mutate(n_diff=replace_na(n_diff,0) %>% 
           max(0)) %>% 
  filter(is.na(lowest_income_served_clean)==F) %>% 
  ggplot(aes(x=lowest_income_served_clean,y=n_diff)) +
  geom_jitter(width=1,height=1) +
  labs(x = "Lowest Income Served (% Area Median Income)",
       y = "Bunching (n_diff metric)",
       title = "Bunching by Lowest Income Served")
```
...
bunching_cities_mod %>% 
  mutate(n_diff=replace_na(n_diff,0) %>% 
           max(0)) %>% 
  filter(is.na(minimum_set_aside_clean)==F) %>% 
  ggplot(aes(x=minimum_set_aside_clean,y=n_diff)) +
  geom_jitter(width=.5,height=.5) +
  labs(x = "Minimum Set-Aside (% of total units)",
       y = "Bunching (n_diff metric)",
       title = "Bunching by Minimum Set-Aside")
```
...
single_eda_plot(jurisdiction="CORAL SPRINGS",state="FL")
```
...
df_sf_hist %>% 
  filter(number_of_units>=3,number_of_units<=100) %>% 
  ggplot() +
  geom_bar(aes(x=number_of_units,y=freq),stat='identity',color='white') +
  ...
  labs(x = 'Number of Units per Building',
       ...
       title = 'Distribution of Apartment Building Unit Counts in SF, 6/2016-4/2023')
```
...
df_sf %>% 
  filter(number_of_units>=3,
         number_of_units<=100,date_built<"2016-06-01") %>% 
  ggplot(aes(x=number_of_units)) + 
  geom_histogram(color='white',binwidth=1) +
  geom_vline(xintercept = 24.5,linetype='dashed',color='blue') +
  geom_vline(xintercept = 9.5,linetype='dashed',color='blue') +
  labs(x = 'Number of Units per Building',y='Number of Buildings',
       title = 'Distribution of Apartment Building Unit Counts in SF, 10/1977-6/2016')
```
...
df_sf_boot = df_sf_hist %>% 
  mutate(residual=freq-counterfactual_freq_pow,
         threshold_num_1 = 10,
          units_below_thresh_1=threshold_num_1-number_of_units,
         bunch_ind_1 = as.numeric((units_below_thresh_1==1)|
           (units_below_thresh_1==2)&(threshold_num_1 %% 2 == 0)),
         pre_bunch_ind_1 = as.numeric(
           ((units_below_thresh_1 %in% c(3,4))&(threshold_num_1 %% 2 == 0))|
           ((units_below_thresh_1 %in% c(2,3))&(threshold_num_1 %% 2 == 1))
           ),
         
         threshold_num_2 = 25,
          units_below_thresh_2=threshold_num_2-number_of_units,
         bunch_ind_2 = as.numeric((units_below_thresh_2==1)|
           (units_below_thresh_2==2)&(threshold_num_2 %% 2 == 0)),
         pre_bunch_ind_2 = as.numeric(
           ((units_below_thresh_2 %in% c(3,4))&(threshold_num_2 %% 2 == 0))|
           ((units_below_thresh_2 %in% c(2,3))&(threshold_num_2 %% 2 == 1))
           ),
         
         
         ) 

n_pre_bunch_obs_2 = df_sf_boot %>% filter(pre_bunch_ind_2==1) %>% pull(freq) %>% 
    sum
n_bunch_obs_2 = df_sf_boot %>% filter(bunch_ind_2==1) %>% pull(freq) %>% 
    sum
n_diff_obs_2 = n_bunch_obs_2-n_pre_bunch_obs_2

n_pre_bunch_obs_c = df_sf_boot %>% filter(pre_bunch_ind_2==1|pre_bunch_ind_1==1) %>% pull(freq) %>% 
    sum
n_bunch_obs_c = df_sf_boot %>% filter(bunch_ind_2==1|bunch_ind_1==1) %>% pull(freq) %>% 
    sum
n_diff_obs_c = n_bunch_obs_c-n_pre_bunch_obs_c

nrows = nrow(df_sf_boot)
boot_diffs_2 = vector(length=n_bootstraps)
boot_diffs_c = vector(length=n_bootstraps)
...
for (i in 1:n_bootstraps){
  ...
  n_pre_bunch_i_2 = df_sf_boot_i %>% filter(pre_bunch_ind_2==1) %>% pull(freq_boot) %>% 
    sum
  n_bunch_i_2 = df_sf_boot_i %>% filter(bunch_ind_2==1) %>% pull(freq_boot) %>% 
    sum
  n_diff_i_2 = n_bunch_i_2-n_pre_bunch_i_2
  boot_diffs_2[i] = n_diff_i_2
  
  n_pre_bunch_i_c = df_sf_boot_i %>% filter(pre_bunch_ind_2==1|pre_bunch_ind_1==1) %>% pull(freq_boot) %>% 
    sum
  n_bunch_i_c = df_sf_boot_i %>% filter(bunch_ind_2==1|bunch_ind_1==1) %>% pull(freq_boot) %>% 
    sum
  n_diff_i_c = n_bunch_i_c-n_pre_bunch_i_c
  boot_diffs_c[i] = n_diff_i_c
}
```

```{r}
n_diff_obs_2
p_val_2 = length(boot_diffs_2[boot_diffs_2>n_diff_obs_2])/length(boot_diffs_2)
p_val_2
```

```{r}
n_diff_obs_c
p_val_c = length(boot_diffs_c[boot_diffs_c>n_diff_obs_c])/length(boot_diffs_c)
p_val_c
```


```{r}
as.data.frame(boot_diffs_2) %>% 
  ggplot() +
  geom_histogram(aes(x=boot_diffs_2),color='white') +
  geom_vline(xintercept = n_diff_obs_2,linetype='dashed') +
  annotate("text",label=str_glue("p = {p_val_2}"),x=11,y=20) +
  labs(title='Distribution of Bootstrapped Test Statistics: SF Bunching Test',
     x = 'Test Statistic',y='Frequency')
```

```{r}
as.data.frame(boot_diffs_c) %>% 
  ggplot() +
  geom_histogram(aes(x=boot_diffs_c),color='white') +
  geom_vline(xintercept = n_diff_obs_c,linetype='dashed') +
  annotate("text",label=str_glue("p = {p_val_c}"),x=21,y=20) +
  labs(title='Distribution of Bootstrapped Test Statistics: SF Bunching Test',
     x = 'Test Statistic',y='Frequency')
```
...
```{r}
df_iz %>% count(`minimum_set-aside`) %>% arrange(desc(n)) %>% head()
```

```{r}
df_iz %>% count(`lowest_income_served`) %>% arrange(desc(n)) %>% head()
```

```{r}
df_analysis %>%
  filter(minimum_set_aside!="Don't know") %>%
  mutate(minimum_set_aside_clean =
           str_extract(minimum_set_aside,pattern="[0-9.]+") %>% 
           as.numeric) %>%
  count(minimum_set_aside_clean) %>%
  arrange(desc(n))
```
