...
df_units %>% 
  mutate(bunch_ind = as.numeric((threshold_num-number_of_units==1)|
         ...
         pre_bunch_ind = as.numeric(
           (((threshold_num-number_of_units) %in% c(3,4))&(threshold_num %% 2 == 0))|
           (((threshold_num-number_of_units) %in% c(2,3))&(threshold_num %% 2 == 1))
           )) %>% 
  filter(situs_city=='PORTLAND',situs_state=='OR') %>% 
  group_by(threshold_num,number_of_units,bunch_ind,pre_bunch_ind) %>% 
  summarize(n())
```
...
single_eda_plot(city="REDMOND",state="WA")
```
...
df_merged %>% 
  ...
  distinct(government_name,state,city_pop) %>% 
  arrange(desc(city_pop))
```
