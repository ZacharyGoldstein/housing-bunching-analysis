...
```{r}
thresh_group_mcrary_results = data.frame(matrix(nrow=1,ncol=6))
names(thresh_group_mcrary_results) = c("group","binsize","theta","se","p","z")
for (group in df_units$threshold_group %>% unique %>% sort){
  df_group = df_units %>% filter(threshold_group==group)
  n_buildings = df_group %>% nrow
  n_programs = df_units %>% select(government_name,situs_state) %>% n_distinct
  ...
  results_vec = c(group,binsize,theta,se,p,z)
  thresh_group_mcrary_results = rbind(thresh_group_mcrary_results,results_vec) 
    
}
thresh_group_mcrary_results = thresh_group_mcrary_results %>% drop_na()
...
for (group in df_units$threshold_group %>% unique %>% sort){
  ...
  theta = results$theta
  print(c(group,p,z,binsize,theta))
}
```
