...
df_units %>% count(threshold_num) %>% 
  ...
  labs(x = 'Unit Threshold',
       ...
       title = 'Distribution of Unit Thresholds')
```
...
```{r}
ggplot(df_units %>% filter(number_of_units<=150),
       aes(x=rel_units)) +
...
```{r}
ggplot(df_units %>% filter(number_of_units<=100,threshold_num<=9),
       aes(x=rel_units)) +
  geom_histogram(binwidth=2,color='white')
```

```{r}
ggplot(df_units %>% filter(number_of_units<=100,threshold_num>=10,threshold_num<=14),
       aes(x=rel_units)) +
  geom_histogram(binwidth=2,color='white')
```

```{r}
ggplot(df_units %>% filter(number_of_units<=100,threshold_num>=15,threshold_num<=19),
       aes(x=rel_units)) +
  geom_histogram(binwidth=2,color='white')
```

```{r}
ggplot(df_units %>% filter(number_of_units<=100,threshold_num>=20,threshold_num<=24),
       aes(x=rel_units)) +
  geom_histogram(binwidth=2,color='white')
```

```{r}
ggplot(df_units %>% filter(number_of_units<=100,threshold_num>=25,threshold_num<=29),
       aes(x=rel_units)) +
  geom_histogram(binwidth=2,color='white')
```

```{r}
ggplot(df_units %>% filter(number_of_units<=100,threshold_num==30),
       aes(x=rel_units)) +
  geom_histogram(binwidth=2,color='white')
```
```{r}
ggplot(df_units %>% filter(number_of_units<=100,threshold_num==50),
       aes(x=rel_units)) +
  geom_histogram(binwidth=2,color='white')
```

```{r}
ggplot(df_units %>% filter(threshold_num==100),
       aes(x=rel_units)) +
...
```{r}
sf_counterfactual_model_ft = plfit(freq_sqrt~number_of_units_sqrt,
                              data=df_sf_hist_train)
df_sf_hist$counterfactual_freq_ft = predict(sf_counterfactual_model_ft,df_sf_hist) %>% round(1)
```
...
df_sf_hist %>% 
  ...
  ggplot() +
  geom_bar(aes(x=number_of_units,y=freq),stat='identity') +
  geom_line(aes(x=number_of_units,y=counterfactual_freq_nb),color='blue') +
  geom_vline(xintercept=23.5,linetype='dashed',color='blue') +
  geom_vline(xintercept=24.5,linetype='dashed',color='black') +
  scale_x_continuous(n.breaks=10) +
  labs(x = 'Number of Units per Building',
...
bootstrap residuals resampling
